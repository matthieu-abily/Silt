(() => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        const obj = clampObjectToActivePage({
          id: uid(), type: 'image', name: safeString(file.name, 512), src: reader.result,
          x: center.x - w / 2 + index * 40, y: center.y - h / 2 + index * 40,
          w, h, rotation: 0,
        });
        state.objects.push(obj);
        state.imageCache.set(obj.id, img);
        setSelection([obj.id]);
        markDirty(); draw(); showToast('Image added');
      };
  ...
  function loadProjectPayload(payload) {
    ...
    ensureImageCache(() => { setClean(); draw(); showToast('Silt board opened'); });
  }

  function saveProject() {
    const payload = serializeProject();
    downloadBlob(JSON.stringify(payload, null, 2), suggestedBoardFilename(), 'application/vnd.silt.board+json');
    setClean();
  ...
  function openProject(file) {
    ...
    const reader = new FileReader();
    reader.onload = () => {
      try { loadProjectPayload(JSON.parse(reader.result)); }
  ...
  window.SiltBridge = {
    ...
    loadProjectJSONString(jsonString) { loadProjectPayload(JSON.parse(jsonString)); return true; },
...
async function registerSiltServiceWorker() {
  ...
  try {
    ...
    if (registration.waiting) {
      registration.waiting.postMessage({ type: "SILT_SKIP_WAITING" });
    }
    ...
    registration.addEventListener("updatefound", () => {
      ...
      worker.addEventListener("statechange", () => {
        if (worker.state === "installed") {
          updatePWAStatus(navigator.serviceWorker.controller ? "Updated" : "Offline ready", "ready");
        }
      });
    });
  } catch (err) {
